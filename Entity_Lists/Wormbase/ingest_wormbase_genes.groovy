...
def final_output = "WORMBASE_GENES_FINAL.txt"
...
ourInputFile.eachLine {
    if (it.startsWith('Gene ')){
        ...
        if (gene.contains('WBGene') && publicName != '') {
            ourOutputFile << gene << "\t" << publicName << "\t" << cgcName << "|" << otherName << "\t" << "\t" << sequenceName << "|" << species << "|" << liveOrDead << "\n"
        }
        //ourOutputFile << "\t" << symbol << "\t\t" << sequence << '|' << species  << '\n'
        gene = ''
        publicName = ''
        cgcName = ''
        otherName = ''
